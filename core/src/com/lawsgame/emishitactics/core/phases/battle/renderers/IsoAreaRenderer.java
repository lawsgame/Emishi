...
public class IsoAreaRenderer extends AreaRenderer {
    ...
    public IsoAreaRenderer(Area model, BattlefieldRenderer bfr, AssetProvider assetProvider) {
        super(model);
                ...
                for (int c = getModel().getColInit(); c < getModel().getColInit() + checkmap[0].length; c++) {
                    ...
                    if (checkmap[r - getModel().getRowInit()][c - getModel().getColInit()]) {
                        ...
                        if(otc){
                            if(wtc){
                                if(!stc){
                                    sprite = new Sprite(assetProvider.getAreaSprite(color, AreaSpriteType.ANTI_SIDE));
                                ...
                                if(stc){
                                    ...
                                    if(getModel().getType().isRectangular()){
                                        sprite = new Sprite(assetProvider.getAreaSprite(color, AreaSpriteType.CORNER_OBTUSE));
                                    }else{
                                        sprite = new Sprite(assetProvider.getAreaSprite(color, AreaSpriteType.ANTI_OUTSIDE_ACUTE));
                            ...
                            if(wtc){
                                if(stc){
                                    if(!this.getModel().getType().isRectangular()) {
                                        sprite = new Sprite(assetProvider.getAreaSprite(color, AreaSpriteType.ANTI_OUTSIDE_OBTUSE));
                                        sprite.setPosition(xCenter - 0.25f, yCenter - 0.5f);
                                        sprite.setFlip(false, true);
                                        sprites.add(sprite);
                                    }
                                    sprite = new Sprite(assetProvider.getAreaSprite(color, AreaSpriteType.ANTI_INSIDE_OBTUSE));
                                    sprite.setFlip(false, true);
                                }else{
                                    sprite = new Sprite(assetProvider.getAreaSprite(color, AreaSpriteType.BORDER));
                        ...
                        if(otc){
                            if(wtc){
                                ...
                                if(!ntc){
                                    sprite = new Sprite(assetProvider.getAreaSprite(color, AreaSpriteType.ANTI_SIDE_UPSIDE_DOWN));
                                    sprite.setFlip(false, true);
                                ...
                                if(ntc){
                                    ...
                                    if(getModel().getType().isRectangular()){
                                        sprite = new Sprite(assetProvider.getAreaSprite(color, AreaSpriteType.CORNER_ACUTE));
                                    }else {
                            ...
                            if(wtc){
                                if(ntc){
                                    ...
                                    sprite = new Sprite(assetProvider.getAreaSprite(color, AreaSpriteType.ANTI_INSIDE_ACUTE));
                                }else{
                                    sprite = new Sprite(assetProvider.getAreaSprite(color, AreaSpriteType.BORDER));
                                }
                            }else{
